...
🔗https://notorecovery-fmdvf9cucdbpgacw.canadacentral-01.azurewebsites.net
...
flowchart TB
    ...
    subgraph NLPPipeline ["NLP Pipeline (Python)"]
        direction TB
        RawData[/Raw News Data/]
        ProcessedData[Cleaned Data]
        
        subgraph Tasks ["Main NLP Tasks"]
            REI[Recovery Elements \nIdentification]
            LR[Location Recognition]
            SA[Sentiment Analysis]
        end
        
        StructuredData[(Structured Geo-Data)]

        RawData -->|Preprocessing| ProcessedData
        ProcessedData --> REI
        ProcessedData --> LR
        ProcessedData --> SA
        REI & LR & SA --> StructuredData
    end
    ...
    Yahoo -.->|Web Crawler| RawData
    StructuredData -.->|Data Ingestion| DB

    %% Style
    style Frontend fill:#FFD6CF,stroke:#333,stroke-width:2px
    style Backend fill:#E3F2FC,stroke:#333,stroke-width:2px
    style NLPPipeline fill:#EBFFEB,stroke:#333,stroke-width:2px
    style DB fill:#fff,stroke:#333,stroke-dasharray: 5 5
    style Tasks fill:#fff,stroke-dasharray: 2 2
```
